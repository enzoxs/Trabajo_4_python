    ...
    def calculo_impuesto(self):
        ...
        for i in range(3):         
            ...
            if(self.empleado[i]["sueldo"]>=10000 and self.empleado[i]["sueldo"]<=30000):
                ...
                comentario = " ".join(("El descuento para el empleado ", self.empleado[i]['nombre'], "es de un 10%", "con un total de descuento de", str(round(self.impuesto))))
                self.c_impuesto.append(comentario) 
                
            elif(self.empleado[i]["sueldo"]>30000 and self.empleado[i]["sueldo"]<=50000):
                self.impuesto=self.empleado[i]["sueldo"]*0.20
                comentario = " ".join(("El descuento para el empleado ", self.empleado[i]['nombre'], "es de un 20%", "con un total de descuento de", str(round(self.impuesto))))
                self.c_impuesto.append(comentario)
                
            elif(self.empleado[i]["sueldo"]>50000):
                self.impuesto=self.empleado[i]["sueldo"]*0.35
                comentario = " ".join(("El descuento para el empleado ", self.empleado[i]['nombre'], "es de un 35%", "con un total de descuento de", str(round(self.impuesto))))
                self.c_impuesto.append(comentario)
            
            d1={"impuesto":round(self.impuesto)}
            self.v_impuesto.append(d1)
        #print(self.v_impuesto) #muestro el valor que tengo en esta opcion
            #self.v_impuesto.append(self.impuesto) 
            
            # aqui guardamos los valores de impuesto de cada uno de los empleados, para sumarlos mas adelante
        #self.menu()
...
#**************************************************** Calculo AFP ****************************************************
    def calculo_afp(self):
        for i in range(3):
            ...
            d2={"afp":round(self.afp)}
            self.v_afp.append(d2)
        #print(self.v_afp)
...
#**************************************************** Calculo Salud ****************************************************    
    def calculo_salud(self):
        for i in range(3):
            ...
            d3={"salud":round(self.salud)}
            self.v_salud.append(d3)
        #print(self.v_salud)
